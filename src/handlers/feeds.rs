...
// `GET /api/feeds` is the JSON shape that `static/js/pages/entries.js` reads
// to populate the feed-icon column on the entries list. The SSR /feeds page
// no longer consumes it, but PR-10 (entries SSR) is the last consumer; the
// endpoint stays alive until then.
...
pub async fn list_feeds(
    ...
    let (mut feeds, categories) = state
        ...
        .read_user(move |c| {
            ...
            Ok::<_, AppError>((feed_dtos, cat_options))
        })
        .await??;
