...
// ============================================================================
// GET /api/entries/{id} — reading-pane deep link
// ============================================================================
...
/// deep links — clean RDRS-native shape, no GReader-protocol coupling.
pub async fn get_entry_detail(
    ...
    let response = state
        .db
        .read_user(move |conn| {
            let ewf = entry::find_by_id_with_feed(conn, id)?.ok_or(AppError::EntryNotFound)?;
            let cat =
                category::find_by_id(conn, ewf.category_id)?.ok_or(AppError::CategoryNotFound)?;
            if cat.user_id != user_id {
                return Err(AppError::EntryNotFound);
            }

            let e = &ewf.entry;
            let link_str = e.link.clone().unwrap_or_default();
            ...
            let content = if let Some(c) = e.content.as_deref() {
                sanitize_html(c, &secret, base_url, referrer, proxy_base_url.as_deref())
            } else {
            ...
            let summary_status = entry_summary::get_statuses_for_entries(conn, user_id, &[id])
                .ok()
            ...
            Ok::<_, AppError>(EntryDetailResponse {
                ...
                feed_title: ewf
                    .feed_title
                    .clone()
                    .unwrap_or_else(|| ewf.feed_url.clone()),
