...
| `RDRS_FETCH_ALLOW_PRIVATE_HOSTS` | - | Comma-separated hostnames, IPs or CIDRs the feed, discovery and icon fetchers may reach even though they resolve inward (e.g. `192.168.0.0/16, nas.local`). Everything private, loopback, link-local or `.local`/`.internal` is refused by default: a feed URL, a `<link rel=alternate>` in someone's HTML and a feed's `<icon>` are all attacker-influenced, and without this the server would fetch them on behalf of whoever asked. Name the hosts here to subscribe to a feed on your own LAN. Non-http(s) URLs stay refused regardless. |
