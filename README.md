...
| `RDRS_FETCH_ALLOW_PRIVATE_HOSTS` | - | Comma-separated hostnames, IPs or CIDRs that feed fetching, discovery, icons, full-content extraction and the image proxy may reach even though they are unreachable from the internet (e.g. `192.168.0.0/16, nas.local`). Everything loopback, private, link-local, CGNAT (`100.64/10`, including Tailscale), multicast or reserved is refused by default, along with `.local`/`.internal` names — a feed URL, a `<link rel=alternate>` in someone's HTML and a feed's `<icon>` are all attacker-influenced, and without this the server would fetch them on behalf of whoever asked. The check is not just on the URL: every redirect hop is re-checked, and a hostname's resolved addresses are filtered at connect time, so neither a `302` into your LAN nor DNS rebinding gets through. Name the hosts here to subscribe to a feed on your own LAN; naming a hostname accepts wherever that name resolves. Non-http(s) URLs stay refused regardless. |
