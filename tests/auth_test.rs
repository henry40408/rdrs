...
/// Age a session by `age`, so it lands inside the sliding-refresh window without
/// a test waiting days for it. The cookie the client holds is unaffected — only
/// the row moves — which is the state a long-lived browser session reaches on
/// its own.
///
/// Takes the raw *cookie value* `<token>.<hmac>`; the database stores only the
/// token, so the signature is stripped here rather than at every call site.
...
/// Create a user directly in the database, bypassing `POST /api/setup` so setup
/// does not itself consume a slot from the client's rate-limit budget —
/// registration is a guarded, never-released endpoint, and going through it here
/// would leave too few attempts for a test that means to exercise login.
...
async fn test_login_rate_limit_does_not_leak_valid_credentials() {
    // Proves the rate-limit check runs before `verify_password`: once the budget
    // is exhausted, even the CORRECT password gets 429. If the check ran after
    // verification, a throttled attacker could distinguish "right password, rate
    // limited" from "wrong password, rate limited".
    let (server, db) = build_server(default_test_config()).await;
    create_user_directly(&db, "admin", "vulture-mango-77-quilt").await;
...
async fn test_successful_login_does_not_consume_the_budget() {
    ...
    for _ in 0..10 {
        let response = server
            ...
            .await;
        response.assert_status_ok();
        // Each login mints a fresh session and CSRF cookie pair, so the header
        // has to be refreshed for the next iteration's POST. `add_header`
        // accumulates rather than replaces, so the stale one must be cleared
        // first — otherwise `X-CSRF-Token` carries two values and the guard reads
        // the oldest. Unrelated to rate limiting.
...
async fn test_registration_budget_exhaustion_does_not_lock_out_login() {
    ...
    // counter. Registration never releases its reservation, so five registrations
    // from one IP left zero budget behind and a subsequent login with the CORRECT
    // password got 429 — indistinguishable from a real lockout. The two must draw
...
async fn passkey_challenge_never_discloses_registered_credentials() {
    // Regression: the sign-in challenge used to be built from every row of the
    // `passkey` table, and webauthn-rs turns the credentials it is given into
    // `allowCredentials` — so one unauthenticated POST returned the credential ID
    // of every passkey on the instance. Those IDs are stable, linkable per-user
    // identifiers. The flow is discoverable now: the challenge names none.
    let (server, db) = build_server(default_test_config()).await;
    create_user_directly(&db, "admin", "vulture-mango-77-quilt").await;
...
/// Both masquerade transitions must hand the client a *new* session cookie and a
/// matching CSRF cookie — the privilege-change renewal OWASP requires. Asserted
/// on the wire rather than in the model layer, because a rotation the handler
/// forgets to reissue would leave the browser authenticated against a row that
/// no longer exists.
#[tokio::test]
...
async fn an_over_long_password_is_refused_before_it_is_hashed() {
    // A rejected password must not reach Argon2, or the length cap would be
    // decided by the request-body limit and an attacker could pick how much
    // hashing work each refused registration costs. Observable only as a
    // validation failure rather than a success, but the ordering is the point.
    let server = create_test_server(default_test_config()).await;
...
async fn a_refused_setup_does_not_hash_the_password() {
    ...
    // runs: Argon2 is deliberately expensive, and paying it for a request that was
    // never going to succeed is free CPU for whoever sends it. Asserted through
    // the refusal it produces, plus the account genuinely not existing after.
    let (server, db) = build_server(default_test_config()).await;
...
async fn test_unprefixed_cookie_still_authenticates_when_secure_enabled() {
    // A cookie_secure = true server only ever *writes* the __Host- prefixed name,
...
async fn test_session_cookie_max_age_matches_sliding_ttl() {
    // The row's expires_at is `now + SESSION_EXPIRY_DAYS`, sliding forward while
    // the session stays active. The cookie's Max-Age must match that, not the
    // 90-day absolute cap the row can eventually reach — otherwise the browser
    // holds a cookie whose backing row is long gone.
    let cookie = login_session_cookie(&create_test_server(default_test_config()).await).await;
    assert!(
...
/// `logout` emits empty removal cookies for both `session_token` and
/// `csrf_token`, and the sliding-cookie middleware — layered outside
/// `anonymous_session` and observing this same response — must yield to them
/// rather than reissuing a live cookie on top, which would silently fail the
/// logout.
#[tokio::test]
...
/// With `cookie_secure = true` the live cookies are minted under the `__Host-`
/// names, so the sliding middleware sees those on the incoming request. The
/// `__Host-` removal cookies are always appended, never gated on `jar.remove()`'s
/// presence check, so `slide_session_cookie`'s "is this purpose already covered"
/// check must recognize its own removal and not append a live cookie alongside
/// it — that would silently undo the logout.
#[tokio::test]
...
/// OWASP's "Manual Session Expiration": logout should instruct the browser to
/// delete data associated with the application. This pins the header value; the
/// deliberate omission of `"cookies"` and `"executionContexts"` is pinned
/// separately below, so a regression in either direction fails loudly.
#[tokio::test]
...
/// Pins the deliberate omission of `"cookies"` (and `"executionContexts"`) from
/// the logout `Clear-Site-Data` header. The server already emits explicit removal
/// cookies, and `Clear-Site-Data` processing is asynchronous relative to JS, so
/// clearing `"cookies"` would race the `flash` cookie `rdrs-flash.js` writes
/// right after this response lands and could eat the logout notice.
#[tokio::test]
