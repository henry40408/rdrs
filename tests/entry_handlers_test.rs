...
// ============================================================================
// GET /api/entries/{id} — reading-pane deep link endpoint
// ============================================================================
...
async fn test_get_entry_detail_owner() {
    ...
    response.assert_status_ok();
    let body: serde_json::Value = response.json();
    assert_eq!(body["id"], entry_ids[0]);
    assert_eq!(body["title"], "Entry Title 1");
    assert_eq!(body["link"], "https://example.com/entry/1");
    assert_eq!(body["feed_title"], "Test Feed");
    assert_eq!(body["category_name"], "Test Category");
    assert!(body["content"]
        .as_str()
        .unwrap()
        .contains("Entry content 1"));
}
...
async fn test_get_entry_detail_not_owner() {
    let app = create_test_app(default_test_config());
    let (_user_id, _cat_id, _feed_id, _entry_ids) = setup_test_data(&app.db).await;
    let (_other_id, _other_cat, _other_feed, other_entries) = setup_second_user_data(&app.db).await;

    login(&app.server).await; // logs in as testuser

    // testuser tries to fetch otheruser's entry
    let response = app
        .server
        .get(&format!("/api/entries/{}", other_entries[0]))
        .await;
    response.assert_status_not_found();
}
...
async fn test_get_entry_detail_missing() {
    ...
    let response = app.server.get("/api/entries/9999").await;
    response.assert_status_not_found();
}
